...
Now that Django is running, you can go to `http://localhost:8001/pl_download/testfetch/` to download the netcdf files for the next few days. (These next few commands take a long time, so read the whole paragraph for shorter alternatives. Also, you can go to the terminal window where celery is running to watch the tasks go by, even after the pageload times out.) Then `http://localhost:8001/pl_plot/testplot/` to plot the files for the next few days. To tile these plots, got to `https://localhost:8001/pl_chop/testchop/`. These will all take a long time. It will go faster if you give your VM multiple cores. You can also use the django console in PyCharm to run functions that only plot one plot, instead of all of them. To plot something, for example, in the console you'd need to do `from pl_plot.models import OverlayManager`, then `OverlayManager.make_plot.delay(1)`. This would plot the sst overlay for the first time index of the most recent netcdf file. Everything is saved to the `media/` directory that's at the same level as your project directory.
