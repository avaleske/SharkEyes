...
def home(request):
    ...
    datetimes = [ i.applies_at_datetime.astimezone(tz.tzlocal()).strftime('%D, %I %p') for i in overlays_view_data.exclude(definition_id=5) ]
    ...
    list_of_wind_times = winddatetimes #don't need to filter since we only have 1 model type

    context = {'overlays': overlays_view_data.exclude(id=4), 'defs': OverlayDefinition.objects.filter(is_base=True).exclude(id=4), 'times':list_of_times, 'windoverlays': wind_overlays_view_data, 'winddefs': OverlayDefinition.objects.filter(id=5), 'windtimes':list_of_wind_times}
