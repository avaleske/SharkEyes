...
def home(request):
    ...
    num_defs = len(OverlayDefinition.objects.filter(is_base=True).exclude(id=4))

    list_of_times = datetimes[:len(datetimes)/num_defs]

    #TODO remove the DEFINITION ID spec in order to display WIND and WAVE items
    context = {'overlays': overlays_view_data, 'defs': OverlayDefinition.objects.filter(is_base=True).exclude(id=4), 'times':list_of_times }
